...
class Cell(object):
    """Kласс "клетка". Здесь мы задаем и визуальное отображение клеток, по визуальному отображению мы
    проверяем какого типа клетка. """
    empty_cell = 'O'
    ship_cell = '■'
    destroyed_ship = 'X'
    damaged_ship = '□'
...
class Field(object):
    ...
    радар на котором игрок отмечает свои ходы и результаты, поле с весом клеток используется для ходов ИИ
    """
    ...
    def check_ship_fits(self, ship, element):
        """Проверяем помещается ли корабль на конкретную позицию конкретного поля.
        Будем использовать при расстановке кораблей, а так же при вычислении веса клеток
        возвращать False если не помещается и True если корабль помещается"""
        field = self.get_field_part(element)
    ...
    def add_ship_to_field(self, ship, element):
        ...
        параметр element - сюда мы передаем к какой части поля мы обращаемся: основная, radar или weight"""
...
class Player(object):
    ...
    def get_input(self, input_type):
        ...
        try:
            ...
            print('Перезапустите игру и посторайтесь вводить коректно координаты')
    ...
    def receive_shot(self, shot):
        """Здесь игрок будет принимать выстрел и будет возвращать результат выстрела
...
if __name__ == '__main__':
    ...
    players = []
    players.append(Player(name=input('Ваше имя: '), is_ai=False, auto_ship=True, skill=1))
    players.append(Player(name='R2 - D2', is_ai=True, auto_ship=True, skill=1))
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            Game.clear_screen()
            game.current_player.message.append("{} Ваш ход:  ".format(game.current_player.name))
            game.draw()
            ...
            if shot_result == 'miss':
                ...
                game.next_player.message.append('По нам попали!')
                continue
